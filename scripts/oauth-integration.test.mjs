...
async function mcpRequest(baseUrl, accessToken, id, method, params = {}) {
  ...
  assert.equal(response.headers.get("x-oneaiworkers-runtime"), "1.0.2");
...
test("OAuth uses S256, rotates refresh tokens, checks resource, revokes, and rate limits", async (t) => {
  ...
  const liveRuntimeCall = await mcpRequest(worker.baseUrl, issued.payload.access_token, 102, "tools/call", {
    name: "w_call",
    arguments: {
      tool_ref: "oneaiworkers:system/runtime_info@1.0.2",
      arguments: {},
    },
  });
  assert.equal(liveRuntimeCall.result.isError, false);
  assert.equal(liveRuntimeCall.result.structuredContent.data.ok, true);
  assert.equal(liveRuntimeCall.result.structuredContent.data.result.result.version, "1.0.2");
