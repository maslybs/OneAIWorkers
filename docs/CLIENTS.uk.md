...
Оновіть або перепідключіть MCP-сервер, щоб клієнт повторно завантажив перелік інструментів. Після цього збережені дії конектора з’являться як звичайні інструменти, наприклад `n8n_list_workflows`.
