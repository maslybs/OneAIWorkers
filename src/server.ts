...
export async function createMcpServer(env: Env, request: Request): Promise<McpServer> {
  ...
  tool(server, "call_connector_tool", "Call connector action", bi("Advanced/debug gateway. Calls a saved connector action by connector_id and action_name. Normal MCP clients should prefer generated tools like tg_getme, tg_send_message, or n8n_list_workflows.", "Розширений шлюз для перевірки. Викликає збережену дію конектора через connector_id і action_name. Звичайні MCP-клієнти мають використовувати створені інструменти на кшталт tg_getme, tg_send_message або n8n_list_workflows."), callConnectorToolSchema, (args) => callConnectorTool(env, args), WRITE_EXTERNAL);
